...
namespace Vicelulas.Dado
{
    public class TriboRepositorio : ITriboRepositorio
    {
        public  IEnumerable<TriboDto> Selecionar()
        {
            ...
            {
                var lista = connection.Query<TriboDto>($"SELECT T.id, T.nome AS Nome, T.ativo, M.id AS IdMentor, P.nome AS NomeMentor FROM [TB_tribo] T " +
        ...
        public TriboDto SelecionarPorId(int id)
        {
            using (var connection = new SqlConnection(DbConnectionFactory.SQLConnectionString))
            {
                var obj = connection.QueryFirstOrDefault<TriboDto>($"SELECT T.id, T.nome AS Nome , T.ativo, M.id AS IdMentor, P.nome AS NomeMentor FROM [TB_tribo] T " +
                                                                   $"LEFT JOIN TB_mentor M ON T.id_mentor = M.id " +
                                                                   $"LEFT JOIN TB_pessoa P ON " +
