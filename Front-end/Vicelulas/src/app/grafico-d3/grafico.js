...
.text(function(d) {return "Developed by Shipeng Sun (sunsp.gis@gmail.com), Institute on the Environment, University of Minnesota, now at University of Illinois Springfield";});
 ...
 d3.json("../viceri/js/grafico.json", function(error, root) {
   console.log(error);
...
d3.json('./viceri/js/grafico.json', function (error, root) {
  ...
  // antigamente --> d3.scale.category10()
...
var resetBubbles = function () {
  ...
  d3.select('#bubbleItemNote').text('D3.js')
...
function activateBubble (d, i) {
  ...
  .duration(d3.event.altKey ? 7500 : 350)
  t.selectAll('.topBubble')
  .attr('cx', function (d, ii) { // tinham dois I's aqui nessa function
    if (i === ii) {
      // Nothing to change
      return oR * (3 * (1 + ii) - 1) - 0.6 * oR * (ii - 1)
    } else {
